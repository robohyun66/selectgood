...
nsim = 1000
sim.settings.list = list(list(mn = mns[[1]], sigma=1, nsim=nsim, type="binseg", mc.cores=4),
                         list(mn = mns[[2]], sigma=1, nsim=nsim, type="binseg", mc.cores=4),
                         list(mn = mns[[3]], sigma=1, nsim=nsim, type="binseg", mc.cores=4),
                         list(mn = mns[[4]], sigma=1, nsim=nsim, type="binseg", mc.cores=4))

resultlist = lapply(sim.settings.list,function(mysetting){ dosim2(mysetting)}, mc.cores=2)
